...
export default async function PostPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      ...
      <div className="mb-6">
        <Link href="/posts/new" className="bg-green-500 text-white px-4 py-2 rounded">Add New Post</Link>
      </div>
          ...
          {posts.map(post => (
            <tr key={post.id} className="border-b">
              <td className="border p-2 text-center">
                <Link href={`/posts/${post.id}`} className="text-blue-600 underline">{post.id}</Link>
              </td>
