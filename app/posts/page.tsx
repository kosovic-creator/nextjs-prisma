  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="flex justify-between items-center mb-6">
        ...
        <div>
          <Link href={"/posts?lang=en"} className="mr-2 underline text-blue-600">EN</Link>
          <Link href={"/posts?lang=sr"} className="underline text-blue-600">SR</Link>
        </div>
      </div>
      <div className="mb-6">
          ...
          {posts.map(post => (
            <tr key={post.id} className="border-b">
              ...
              <td className="border p-2 flex gap-2 justify-center">
                <a href={`/posts/${post.id}/update?lang=${lang}`} className="bg-blue-500 text-white px-2 py-1 rounded">{t("Update", "Izmeni")}</a>
